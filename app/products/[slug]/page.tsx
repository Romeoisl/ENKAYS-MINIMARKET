    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 px-4 py-8">
        <div className="grid gap-8 md:grid-cols-2">
          ...
          <div>
            {product.brand && (
              <p className="text-sm font-medium text-enkays-600">{product.brand.name}</p>
            )}
            ...
            <p className={cn("mt-2 text-sm font-medium", `stock-${stock.tone}`)}>
              {stock.label}
            </p>

            {product.shortDescription && (
              <p className="mt-4 text-ink-700">{product.shortDescription}</p>
            )}

            <div className="mt-6 flex flex-wrap gap-3">
              {product.salesMethod === "WHATSAPP" && settings?.whatsappNumber && (
                <WhatsAppButton
                  whatsappNumber={settings.whatsappNumber}
                  siteName={settings.siteName ?? SITE_NAME}
                  productName={product.name}
                  price={product.priceVisibility === "SHOW_PRICE" ? product.price : null}
                  sku={product.sku}
            ...
            <dl className="mt-6 grid grid-cols-2 gap-2 text-sm text-ink-500">
              <dt>SKU</dt>
              <dd className="text-ink-900">{product.sku}</dd>
              {product.category && (
                <>
                  <dt>Category</dt>
                  <dd className="text-ink-900">{product.category.name}</dd>
                </>
              )}
            </dl>
          </div>
