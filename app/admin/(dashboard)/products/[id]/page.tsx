...
export default async function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [product, categories, brands] = await Promise.all([db.product.findUnique({ where: { id }, include: { images: { orderBy: { position: "asc" } } } }), db.category.findMany({ where: { active: true }, select: { id: true, name: true }, orderBy: { position: "asc" } }), db.brand.findMany({ where: { active: true }, select: { id: true, name: true }, orderBy: { name: "asc" } })]);
  if (!product) notFound();
