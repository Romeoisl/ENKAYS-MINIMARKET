...
export default async function HomePage() {
  ...
  return (
    <main className="min-h-screen bg-[#f7faf8] pb-20 md:pb-0">
      ...
      <section className="enkays-section mx-auto max-w-7xl px-5 py-14 sm:px-8 lg:py-20" aria-labelledby="about-enkays">
        <div className="grid gap-10 lg:grid-cols-[1.05fr_.95fr] lg:items-start">
          <div>
            <p className="text-xs font-black uppercase tracking-[.2em] text-emerald-700">Welcome to ENKAYS</p>
            <h2 id="about-enkays" className="mt-2 text-3xl font-black tracking-tight text-[#10231a] sm:text-4xl">A simpler way to find everyday food essentials.</h2>
            <p className="mt-5 text-base leading-8 text-slate-600">
              Enkays Foods & More is a Nigerian foodstuff marketplace built around a simple idea: make everyday shopping easier.
              We bring pantry staples and food essentials into one easy-to-browse store, so you can discover what you need,
          ...
          <div className="rounded-[2rem] border border-emerald-950/5 bg-white p-6 shadow-sm sm:p-8">
            <h3 className="text-xl font-black text-[#10231a]">How ENKAYS works</h3>
            <ol className="mt-6 space-y-5">
              <li className="flex gap-4"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-emerald-50 text-sm font-black text-emerald-700">1</span><div><strong className="block text-sm text-[#10231a]">Browse</strong><span className="text-sm leading-6 text-slate-500">Explore categories and discover products that fit what you need.</span></div></li>
              <li className="flex gap-4"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-emerald-50 text-sm font-black text-emerald-700">2</span><div><strong className="block text-sm text-[#10231a]">Confirm</strong><span className="text-sm leading-6 text-slate-500">Check the product details and contact ENKAYS to confirm availability and ordering.</span></div></li>
              <li className="flex gap-4"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-emerald-50 text-sm font-black text-emerald-700">3</span><div><strong className="block text-sm text-[#10231a]">Order</strong><span className="text-sm leading-6 text-slate-500">Continue through WhatsApp or phone for a direct, human ordering experience.</span></div></li>
            </ol>
          </div>
        </div>
      </section>

      <section id="categories" className="enkays-section mx-auto max-w-7xl px-5 py-14 sm:px-8 lg:py-20"><div className="flex items-end justify-between gap-6"><div><p className="text-xs font-black uppercase tracking-[.2em] text-emerald-700">Shop by need</p><h2 className="mt-2 text-3xl font-black tracking-tight text-[#10231a] sm:text-4xl">Foodstuff categories</h2><p className="mt-2 max-w-xl text-sm leading-6 text-slate-500">Find the staples you already love, then message us to confirm availability and order.</p></div><Link href="/products" className="hidden rounded-full border border-emerald-900/10 px-4 py-2 text-sm font-bold text-emerald-700 transition hover:bg-emerald-50 sm:block">View all</Link></div><div className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">{categories.map((category) => <Link key={category.id} href={`/products?category=${encodeURIComponent(category.slug)}`} className="enkays-card rounded-3xl border border-emerald-950/5 bg-white p-5 shadow-sm"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-emerald-50 text-lg text-emerald-700">✦</span><h3 className="mt-7 font-extrabold text-[#10231a]">{category.name}</h3><p className="mt-1 line-clamp-2 text-xs leading-5 text-slate-500">{category.description ?? "Everyday food essentials selected for you."}</p></Link>)}</div></section>

      <section id="popular" className="enkays-section bg-white py-14 sm:py-20"><div className="mx-auto max-w-7xl px-5 sm:px-8"><div className="flex items-end justify-between gap-6"><div><p className="text-xs font-black uppercase tracking-[.2em] text-emerald-700">Popular now</p><h2 className="mt-2 text-3xl font-black tracking-tight text-[#10231a] sm:text-4xl">What people are looking for</h2></div><Link href="/products" className="rounded-full border border-emerald-900/10 px-4 py-2 text-sm font-bold text-emerald-700 transition hover:bg-emerald-50">Shop everything</Link></div><div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">{products.map((product) => { const image = product.images[0]; const url = product.salesMethod === "WHATSAPP" && whatsapp ? whatsappUrl(whatsapp, `Hello Enkays Foods & More, I’m interested in ${product.name}. Is it available?`) : `/products/${product.slug}`; return <article key={product.id} className="enkays-card overflow-hidden rounded-3xl border border-slate-200 bg-white"><Link href={`/products/${product.slug}`} className="block"><div className="relative aspect-square overflow-hidden bg-slate-100">{image ? <Image src={image.secureUrl} alt={image.alt ?? product.name} fill sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 25vw" className="object-cover transition duration-500 hover:scale-[1.03]" /> : <div className="grid h-full place-items-center text-sm font-bold text-slate-400">ENKAYS</div>} {product.featured && <span className="absolute left-3 top-3 rounded-full bg-amber-300 px-2.5 py-1 text-[10px] font-black uppercase tracking-wider text-[#173223]">Popular</span>}</div><div className="p-4"><p className="text-[11px] font-bold uppercase tracking-wider text-emerald-700">{product.category?.name ?? "Foodstuff"}</p><h3 className="mt-1 line-clamp-2 min-h-10 font-extrabold text-[#10231a]">{product.name}</h3>{product.priceVisibility === "SHOW_PRICE" ? <p className="mt-3 text-base font-black text-[#10231a]">{money(product.price)}</p> : <p className="mt-3 text-sm font-bold text-[#10231a]">Contact for price</p>}</div></Link>{product.salesMethod === "WHATSAPP" && whatsapp && <div className="px-4 pb-4"><a href={url} className="block rounded-full bg-[#0b4631] px-4 py-2.5 text-center text-xs font-bold text-white transition hover:bg-[#073523]">Order on WhatsApp</a></div>}{product.salesMethod === "PHONE" && phone && <div className="px-4 pb-4"><CallOrderButton phoneNumber={phone} /></div>}</article>; })}</div></div></section>
