...
export function ProductForm({ product, categories = [], brands = [] }: { product?: Product; categories?: Option[]; brands?: Option[] }) {
  ...
  return <form onSubmit={submit} className="space-y-6">
    <section className="rounded-2xl border border-ink-100 bg-white p-6 shadow-sm"><div className="grid gap-5 md:grid-cols-2">
      ...
      <label className="space-y-2"><span className="text-sm font-semibold">Sales method</span><select value={form.salesMethod ?? "WHATSAPP"} onChange={(e) => set("salesMethod", e.target.value)} className="w-full rounded-xl border border-ink-200 bg-white px-3 py-2.5"><option value="WHATSAPP">WhatsApp</option><option value="CONTACT">Contact form</option><option value="PHONE">Phone</option><option value="CHECKOUT">Checkout</option><option value="DISABLED">Disabled</option></select></label>
