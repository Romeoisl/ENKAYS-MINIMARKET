...
export function Sidebar() {
  return (
    <aside className="w-60 shrink-0 border-r border-ink-100 bg-white p-4">
      <p className="mb-6 text-sm font-bold text-enkays-700">{SITE_NAME}</p>
      <nav className="space-y-5 text-sm">
        {GROUPS.map((group) => <div key={group.title}><p className="mb-1 px-3 text-[11px] font-semibold uppercase tracking-wider text-ink-400">{group.title}</p>{group.links.map((link) => <Link key={link.href} href={link.href} className="block rounded-lg px-3 py-2 text-ink-700 hover:bg-ink-100">{link.label}</Link>)}</div>)}
      </nav>
    </aside>
  );
}
