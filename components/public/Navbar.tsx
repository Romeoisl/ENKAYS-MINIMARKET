    ...
    <header className="sticky top-0 z-40 border-b border-ink-100 bg-white/95 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center gap-4 px-4 py-3">
        ...
        <nav className="hidden items-center gap-6 text-sm font-medium text-ink-700 md:flex">
          ...
          <Link href="/deals" className="hover:text-enkays-600">Deals</Link>
        </nav>
        ...
        <div className="ml-auto flex items-center gap-4 md:ml-0">
          <Link href="/wishlist" aria-label="Wishlist"><Heart size={22} /></Link>
        </div>
      </div>
      ...
      {open && (
        <div className="border-t border-ink-100 bg-white px-4 py-4 md:hidden">
          <nav className="flex flex-col gap-1 text-sm font-medium text-ink-700">
            ...
            <Link href="/deals" onClick={() => setOpen(false)} className="rounded-lg px-3 py-3 hover:bg-ink-50">Deals</Link>
            <Link href="/wishlist" onClick={() => setOpen(false)} className="rounded-lg px-3 py-3 hover:bg-ink-50">Wishlist</Link>
          </nav>
