...
export type WhatsAppProduct = { name: string; price?: number; sku?: string; url?: string; quantity?: number; variant?: string };
function normalizeNumber(value: string) { return value.replace(/[^\d]/g, ""); }
...
export async function buildProductWhatsAppUrl(product: WhatsAppProduct) {
  const settings = await db.siteSettings.findUnique({ where: { id: 1 } });
  if (!settings?.whatsappNumber) throw new Error("WHATSAPP_NOT_CONFIGURED");
  const lines = ["Hello, I’m interested in ordering from Enkays Foods & More.", "", `Product: ${product.name}`];
