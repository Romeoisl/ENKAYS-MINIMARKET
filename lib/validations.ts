...
export const orderSchema = z.object({
  ...
  deliveryMethod: z.string().trim().max(80).optional().nullable(),
...
export const promotionSchema = z.object({
  ...
  active: z.boolean().default(true),
}).refine((v) => v.endsAt > v.startsAt, { message: "Promotion end must be after start" });

export const couponSchema = z.object({
  code: z.string().trim().min(3).max(40).toUpperCase(),
  type: z.enum(["PERCENTAGE", "FIXED"]),
  value: z.number().int().positive(),
  minimumOrder: z.number().int().nonnegative().optional().nullable(),
...
export const deliverySchema = z.object({
  ...
  estimatedDelivery: z.string().trim().max(160).optional().nullable(),
...
export const settingsSchema = z.object({
  ...
  defaultWhatsAppMessage: z.string().max(2000).optional().nullable(),
  includePriceInWhatsApp: z.boolean(),
  includeSkuInWhatsApp: z.boolean(),
  includeProductUrlInWhatsApp: z.boolean(),
  seoTitle: z.string().max(180).optional().nullable(),
  seoDescription: z.string().max(320).optional().nullable(),
  orderPrefix: z.string().trim().min(1).max(10),
});
